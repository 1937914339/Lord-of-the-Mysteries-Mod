...
import com.wan.gmmod.client.render.BlackVestRenderer;
 ...
 */
public class BlackVestItem extends ArmorItem implements GeoItem {
     ...
     */
    public BlackVestItem(Holder<ArmorMaterial> material, Type type, Properties properties) {
     ...
     * 注册动画控制器
     * <p>
     * 黑色马甲（白衬）暂无复杂动画需求，可在此处后续扩展动画控制器
     *
     * @param controllers 动画控制器注册器
     */
    @Override
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        // 黑色马甲（白衬）目前不需要动画，可按需在此添加 AnimationController
    }
     ...
     * 自定义的 {@link BlackVestRenderer}（继承 {@link GeoArmorRenderer}），
     * 由 GeckoLib 的 {@code HumanoidArmorLayerMixin} 自动接管盔甲渲染。
     * <p>
     * {@code getGeoItemRenderer} 保持默认返回 null，
     * 物品栏 / 手持渲染使用原版物品模型 JSON（{@code models/item/black_vest.json}）。
     *
    ...
    public void createGeoRenderer(Consumer<GeoRenderProvider> consumer) {
        consumer.accept(new GeoRenderProvider() {
            private BlackVestRenderer renderer;
            ...
            public <T extends LivingEntity> HumanoidModel<?> getGeoArmorRenderer(
                ...
                if (this.renderer == null)
                    this.renderer = new BlackVestRenderer();
